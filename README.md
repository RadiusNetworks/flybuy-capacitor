...
if (!customer) {
  ...
  const { customer: newCustomer } = await Flybuy.createCustomer({
    customerInfo: { name: 'Marty McFly', phone: '555-5555' },
    termsOfService: true,
    ageVerification: true,
  });
  // Store token securely for future logins
  await SecureStorage.set('flybuyToken', newCustomer.token);
}

// Restore session on subsequent launches
const token = await SecureStorage.get('flybuyToken');
if (token) {
  await Flybuy.loginWithToken({ token });
}
